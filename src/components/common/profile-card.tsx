  ...
  return <div className={cn("bordered-card rounded-xl p-6", className)}>
      <div className={cn("flex", isCompact ? "flex-col items-center text-center" : "items-start")}>
        <div className="relative">
          <img src={isEditing ? avatarPreview || editedProfile?.avatar_url || profile.avatar_url : profile.avatar_url} alt={profile.full_name} className={cn("rounded-full object-cover", isCompact ? "w-20 h-20 mb-4" : "w-24 h-24 mr-6")} />
          
          {isEditing && <div className="absolute bottom-0 right-0">
        ...
        <div className="flex-1 px-0 mx-0">
          {isEditing ? <div className="space-y-4">
              ...
              <h2 className="font-semibold text-base">{profile.full_name}</h2>
              <p className="text-muted-foreground mb-2 text-xs">@{profile.username}</p>
              <p className="mb-4 text-xs">{profile.about}</p>
              
              <div className="flex gap-6">
                <div>
                  <span className="font-medium text-base">{profile.following_count || 0}</span>
                  <span className="text-muted-foreground ml-1 text-sm">Following</span>
                </div>
                <div>
                  <span className="font-medium text-base">{profile.followers_count || 0}</span>
        ...
        {isCurrentUser && <div className="flex gap-2 mt-4 md:mt-0">
            {isEditing ? <>
              ...
              </> : <Button variant="outline" onClick={onEdit} className="text-sm font-normal px-px py-0 my-0">Edit</Button>}
          </div>}
      </div>
    </div>;
}